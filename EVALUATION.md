...
- **추가 개선(코드 레벨 근거 검증, 2026-09-18)**: 특허 카드의 `_is_valid_item()`과 같은 방식으로 `_is_grounded_benchmarking_item()`을 도입했습니다 — 모델 응답의 `url`이 실제 입력 데이터(뉴스/특허)에 존재하는지, `evidenceQuote`(원문에서 그대로 인용하도록 새로 강제한 필드)가 해당 원문(또는 신차 스펙 텍스트) 안에 실제로 등장하는지 코드로 대조해, 지어낸 근거를 가진 항목은 자동 폐기합니다. 또한 `factLevel`(직접 인용/간접 서술/추론 자기평가) 필드를 추가해 화면에 배지로 노출하고, `internalCheckQuestion` 템플릿 강제·수치 명시 규칙·마케팅 상투어 블랙리스트(`BENCHMARKING_BANNED_PHRASES`)를 프롬프트에 반영해 문구 품질을 높였습니다.
- **한계(개선 후에도 남는 부분)**: 위 검증은 "모델이 근거로 든 url/인용문이 입력 데이터에 실제로 존재하는가"만 확인하며, 근거 데이터(신차 스펙 등) 자체의 사실 정확도(1-5의 한계)와 "이게 진짜 주목할 만한 기술인가"라는 정성적 판단은 여전히 프롬프트 차원의 지시일 뿐입니다. `factLevel`도 모델의 자기 신고이므로 코드가 그 정확성 자체를 검증하지는 못합니다.
...
- 배터리 벤치마킹 포인트의 "유의미한 기술 판단" 근거 검증 — **부분 개선 (2026-09-18)** (1-10 참고. `_is_grounded_benchmarking_item()`으로 url/evidenceQuote가 실제 입력 데이터에서 유래했는지는 코드로 대조하지만, 근거 데이터 자체의 사실 정확도와 "이게 진짜 주목할 만한가"라는 정성적 판단은 여전히 프롬프트 차원의 지시일 뿐 코드 레벨 검증 대상이 아님)
