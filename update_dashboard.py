...
- abstract는 개략적 요약일 뿐이며, 정량적 수치/조성비/치수/온도/시간 등 구체적 설계 데이터는
  claims 필드에 훨씬 상세히 담겨 있다. painPoint/solution/designCriteria를 작성할 때는
  abstract보다 claims를 우선 근거로 삼아라.
...
- headline: 특허의 핵심 기술을 압축한 "완전한 한 문장" 기술 개요 (1행). "기술명 (부연설명)" 같은
  요약어+괄호 형태로 쓰지 말고, 하나의 자연스러운 문장으로 서술하라
  (예: "고전압 작동 환경에서 전극 계면에 안정적인 보호 피막을 형성하는 고안정성 액상 전해액 및 첨가제 기술").
- painPoint: 이 특허가 해결하려는 기존 기술의 구체적 한계/문제점을 1~2문장으로 서술하라
  (예: 망간 용출, 계면 저항 증가, 젤리롤 변형, 열폭주 전이, 전해액 산화 분해 등 claims/abstract에
  근거한 실제 문제만 기술).
- solution: 이 특허만의 구체적 기술적 해결 원리를 1~2문장으로 서술하라 (화학 조성비, 표면 코팅,
  제어 회로, 가압 구조 등 claims/abstract에 근거해 구체적으로).
   * painPoint와 solution 모두 "성능을 향상시킨", "안정성을 높인", "우수한 특성을 지닌" 같은 추상적
     표현은 절대 사용하지 마라.
   * painPoint와 solution에는 정량적 수치를 넣지 마라 (수치는 아래 designCriteria에서만 다룬다).
...
- designCriteria: "핵심 정량 수치 요약(Data Summary)" - claims(우선) 또는 abstract에 실제로 등장하는
   ...
   * group: 파라미터 성격에 따라 아래와 같이 "동적으로" 분류하라 (아래는 예시일 뿐, 특허마다 실제
     담긴 데이터에 맞는 그룹만 만들어라 - 없는 그룹은 절대 만들지 마라):
     - "재료/입도/조성" (예: Dv50, Dv99, 코팅층 비율, 실리콘 함량 등)
     - "공정/물성 타깃" (예: 압축 밀도, 두께, 기공율, 저항률 등)
     - "전해액/화학식" (예: 첨가제 비율, 리튬염 농도, 용매 비 등)
     - "성능/운전 조건" (예: 급속 충전 시간, SOC 범위, 동작 온도 등)
     - 위 4개 중 어디에도 맞지 않는 정량 데이터가 있다면 내용에 맞는 새 그룹명을 만들어도 된다.
   * label: 파라미터 명칭을 원문 그대로 간결히 (예: "Dv50", "압축 밀도", "리튬염 농도").
   * value: claims/abstract에 명시된 구체적 수치/범위/비율을 단위와 함께 원문 그대로 인용하듯 작성하라.
